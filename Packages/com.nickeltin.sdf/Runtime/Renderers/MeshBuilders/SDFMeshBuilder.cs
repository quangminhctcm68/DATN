    ...
    internal abstract class SDFMeshBuilder
    {
            ...
            Vector2 uvMin, Vector2 uvMax, float z = 0, float sdfFlag = 0, float sdfWidth = 0)
        {
            ...
            vh.AddVert(new Vector3(posMin.x, posMin.y, z), color,
                new Vector4(uvMin.x, uvMin.y, sdfWidth, sdfFlag));
            vh.AddVert(new Vector3(posMin.x, posMax.y, z), color,
                new Vector4(uvMin.x, uvMax.y, sdfWidth, sdfFlag));
            vh.AddVert(new Vector3(posMax.x, posMax.y, z), color,
                new Vector4(uvMax.x, uvMax.y, sdfWidth, sdfFlag));
            vh.AddVert(new Vector3(posMax.x, posMin.y, z), color,
                new Vector4(uvMax.x, uvMin.y, sdfWidth, sdfFlag));

            vh.AddTriangle(startIndex, startIndex + 1, startIndex + 2);
            vh.AddTriangle(startIndex + 2, startIndex + 3, startIndex);
        }
        
        public static void AddQuad(LayerInfo layer, Vector2 posMin, Vector2 posMax, Vector2 uvMin, Vector2 uvMax)
        {
            posMin += layer.MeshOffset;
            posMax += layer.MeshOffset;
            AddQuad(layer.VertexHelper, posMin, posMax, layer.Color, uvMin, uvMax, 
