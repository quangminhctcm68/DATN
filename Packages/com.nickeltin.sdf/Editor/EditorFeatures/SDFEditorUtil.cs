...
namespace nickeltin.SDF.Editor
{
    ...
    {
        public const string DEFAULT_GENERATION_BACKEND = "legacy";
        ...
        /// <summary>
        /// Checks <see cref="IsValidTextureType"/> if so then loads sdf import settings and checks <see cref="SDFImportSettings.GenerateSDF"/>.
        /// If returned true that means texture should have sdf generated unless something goes wrong in import process.
        /// </summary>
        [SDFPipelineCompatible(SDFPipelineFlags.RegularAnd2023)]
        public static bool ShouldImportSDF(TextureImporter importer, out SDFImportSettings settings)
        {
            settings = new SDFImportSettings();
            ...
            settings = SDFImportSettings.Load(importer);
            return settings.GenerateSDF;
        }

        /// <inheritdoc cref="ShouldImportSDF(UnityEditor.TextureImporter,out nickeltin.SDF.Editor.SDFImportSettings)"/>
        /// <remarks>
        ///     Checks is main asset is texture, and loads texture importer.
        /// </remarks>
        [SDFPipelineCompatible(SDFPipelineFlags.RegularAnd2023)]
        public static bool ShouldImportSDF(string texturePath, out TextureImporter importer, out SDFImportSettings settings)
        {
            settings = new SDFImportSettings();
            importer = null;
        ...
        public static void FixTextures(Texture[] textures)
        {
            ...
            var defaultSettings = new SDFImportSettings();
        ...
        /// </summary>
        [SDFPipelineCompatible(SDFPipelineFlags.RegularAnd2023)]
        public static void SetImportSettings(Texture[] textures, SDFImportSettings settings)
        {
            var state = ImportSettingsState.CreateStaticState(textures);
            foreach (var container in state)
            {
        ...
        /// <summary>
        /// Before directly loading the assets you might want to use <see cref="ShouldImportSDF(UnityEditor.TextureImporter,out nickeltin.SDF.Editor.SDFImportSettings)"/>
        ...
        /// Works only for regular pipeline, to search for decoupled pipeline use <see cref="FindAllSDFAssets"/>. 
        /// </summary>
        /// <returns></returns>
        [SDFPipelineCompatible(SDFPipelineFlags.RegularAnd2023)]
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            // Sprite is either sdf or source sprite from regular pipeline (part of texture)
            ...
            {
                // Sprite is sdf sprite from decoupled pipeline (part of SDFAsset)
        ...
        /// <summary>
        /// This is very tricky method and its returns result might not be consistent.
        /// It works relatively straightforward for <see cref="SDFPipelineFlags.Regular"/> pipeline
        /// since all required data resides inside on main texture asset.
        /// Also it works fine if <paramref name="sprite"/> is part of <see cref="SDFPipelineFlags.DecoupledSDFSprite"/>,
        /// since metadata can be extracted easy do to sdf sprite and its meta asset is also inside one asset.
        ///
        ...
        /// will be lookup of all <see cref="SDFAsset"/>'s in project and using first that uses <paramref name="sprite"/> texture
        /// as source texture (<see cref="SDFAssetImporter.Texture"/>).
        /// Lookup does not loads all assets so its relativity quick, however its result is not guaranteed
        ...
        /// <param name="metadataAsset">Output meta asset generated either from regular or decoupled pipeline</param>
        ...
        ///     And not sdf sprite from decoupled pipeline (<see cref="SDFPipelineFlags.DecoupledSDFSprite"/>)
            ...
            out SDFSpriteMetadataAsset metadataAsset, out SDFPipelineFlags flags, out string firstSDFAssetGUID)
        {
            ...
            // At this point sprite can either be: Source sprite or SDF sprite from regular pipeline,
            // or any sprite from decoupled pipeline.
            // Assuming we dealing with sprite (sdf or source) from regular pipeline
            // Then check is texture type and import settings is valid to import sdf (therefore sdf should be imported)
            ...
            // If we dealing with sdf sprite from decoupled pipeline then its easy.
            if (flags.HasFlag(SDFPipelineFlags.DecoupledSDFSprite))
            {
                ...
                return sprite.TryGetSpriteMetadataAsset(out metadataAsset);
#else
                // For older version we need to load SDFAsset and then lookup for meta asset
            ...
            // Therefore only way this sprite anyhow involved in any sdf pipeline is by being source sprite for
            // any SDFAsset in project, so all we can do here is try to search for the first sdf asset that has this
            // sprite texture used as source.
        ...
        {
            // Don't need to search for sdf asset since sdf sprite is either sdf sprite from regular pipeline
            // or sdf sprite from SDFAsset, in both cases we have main asset for metadata.
        ...
        {
            // Don't need to search for SDFAsset's since in sdf pair at-least one sprite is sdf sprite,
        ...
        internal static void TryRefreshSDFSpriteReferenceFields(string path, Object target, IEnumerable<string> propertyPaths)
        {
            ...
            foreach (var spriteReferencePath in propertyPaths)
            {
                ...
                // Getting old value, its still valid even if object is null, since its C++ side still exist (apparently)
        ...
        public static void TryRestoreSerialization(SDFImage image)
        {
         ...
         ///    - Texture
         ///    - Folder
         /// For first two its simple, just add sprite or load all sprites.
         /// For folder it requires recursive search for all sprites in folders.
         ///
         /// NOTE:
         ///    If sprites in atlas is from regular sdf pipeline it will return basically sdf-source sprite pairs.
         ///    So if that sprites were used in conversion to <see cref="SDFSpriteMetadataAsset"/> filter them first.
         /// </summary>
         public static IEnumerable<Sprite> LoadSpritesFromAtlas(SpriteAtlas atlas)
         {
             ...
             // Sprites - if sprite encountered simply add it to list
             ...
             // Folders - check is asset folder, add it paths to load textures from it later
