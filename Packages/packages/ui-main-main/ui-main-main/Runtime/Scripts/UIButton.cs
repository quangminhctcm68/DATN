...
namespace NabaGame.UI
{
    ...
    public class UIButton : MonoBehaviour, IPointerClickHandler, ISelectHandler, IDeselectHandler
    {
        ...
        private Coroutine cClickRegistered;
        /// <summary>
        ...
        void OnDisable()
        {
            ...
            //Unity kills the click coroutine when this GameObject is deactivated ->
            //reset its state so the next click (after re-activation) is registered correctly
            if(cClickRegistered != null)
            {
                StopCoroutine(cClickRegistered);
                cClickRegistered = null;
            }
            clickedOnce = false;
        ...
        void RegisterClick()
        {
            //a coroutine cannot be started on an inactive/disabled GameObject (Unity throws an error);
            //this happens when a click handler deactivates this button's GameObject
            if(!gameObject.activeInHierarchy || !enabled)
            {
                if(debug) { Debug.Log("[UIButton] - " + name + " | RegisterClick skipped (inactive GameObject)"); }
                return;
            }
            if(cClickRegistered != null) { StopCoroutine(cClickRegistered); }
            cClickRegistered = StartCoroutine(ClickRegistered());
        }
        IEnumerator ClickRegistered()
        {
            ...
            clickedOnce = false;
            cClickRegistered = null;
            //ExecuteClick may have deactivated this GameObject -> don't start another coroutine on it
            if(!allowMultipleClicks && gameObject.activeInHierarchy && enabled) { DisableButtonAfterClick(); }
        }
        /// <summary>
