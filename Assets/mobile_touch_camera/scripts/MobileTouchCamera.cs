...
#pragma warning restore 649
        ...
        [Tooltip(
            "Here you can define all platforms that should support keyboard and mouse input for controlling the camera.")]
        [SerializeField]
        private List<RuntimePlatform> keyboardAndMousePlatforms = new List<RuntimePlatform>()
        {
            RuntimePlatform.WindowsEditor, RuntimePlatform.WindowsPlayer, RuntimePlatform.OSXEditor,
            RuntimePlatform.OSXPlayer, RuntimePlatform.LinuxEditor, RuntimePlatform.LinuxPlayer,
        ...
        private List<KeyCode> keyboardControlsModifiers = new List<KeyCode>()
        ...
        [SerializeField]
        [Tooltip(
            "When setting this flag to true, the uniform camOverdragMargin will be overrided with the values set in camOverdragMargin2d.")]
...
//        { get; set; }
        ...
        private float
            maxHorizonFallbackDistance =
                10000; //This value may need to be tweaked when using the camera in a low-tilt, see above the horizon scenario.
        ...
        public void Awake()
        {
            ...
            {
                Debug.LogWarning("The defined max camera zoom (" + CamZoomMax + ") is smaller than the defined min (" +
            ...
            if (IsPinching == true)
            {
                if (isTiltModeEvaluated == true)
                {
                    ...
                    if (isPinchTiltMode == false || isPinchModeExclusive == false)
                    {
                        ...
                        // --- Zoom (PHẦN MỚI BẬT LÊN) ---
                        // Tăng khoảng cách 2 ngón -> zoom out, giảm -> zoom in
                        if (pinchDistanceCurrentLerp > 0.01f)
                        {
                            Debug.Log("Zoom!");
                            float zoomFactor = pinchDistanceStart /
                                               Mathf.Max(pinchDistanceCurrentLerp * customZoomSensitivity, 0.01f);

                            float cameraSize = pinchStartCamZoomSize * zoomFactor;

                            // Giới hạn trong min / max để không bị giật khi chạm biên
                            cameraSize = Mathf.Clamp(cameraSize, camZoomMin, camZoomMax);
                    ...
                    // --- Position update (theo tilt / pinch center) ---
                    DoPositionUpdateForTilt(false);
                }
            }
            else
            {
                // Spring back tilt nếu có overtilt
        ...
        private void UpdatePosition(float deltaTime)
        {
            ...
            if (IsDragging == true || IsPinching == true)
            {
                ...
                if (IsSmoothingEnabled == true)
                {
                    Transform.position = Vector3.Lerp(Transform.position, targetPositionClamped,
                        Mathf.Clamp01(Time.unscaledDeltaTime * camFollowFactor));
                }
                else
                {
            ...
            if (IsDragging == false && IsPinching == false)
            {
                Vector3 overdragSpringVector =
                    ComputeOverdragSpringBackVector(camPos, CamOverdragMargin2d, ref cameraScrollVelocity);
        ...
        /// </summary>
        private Vector3 ComputeOverdragSpringBackVector(Vector3 camPos, Vector2 margin,
        ...
        public void ComputeCamBoundaries()
        {
            ...
            else
            {
                ...
                Vector2 camUp =
                    GetIntersection2d(Cam.ScreenPointToRay(new Vector3(Screen.width * 0.5f, Screen.height, 0)));
        ...
        public void SetCamPosMax(Vector2 value)
        {
            CamPosMax = value;
        }

        public void SetCamPosMin(Vector2 value)
        {
            CamPosMin = value;
        }
        ...
        public void Update()
        {
            ...
            if (cameraScrollVelocity.sqrMagnitude > float.Epsilon)
            {
                ...
                Vector3 camVelDamp = dampFactor * cameraScrollVelocity.normalized * autoScrollDamp *
        ...
        public void LateUpdate()
        {
            ...
            if (keyboardAndMousePlatforms.Contains(Application.platform) && Input.touchCount == 0)
            {
                ...
                if (Mathf.Approximately(mouseScrollDelta, 0) == false)
                {
                    if (isEditorInputRotate == true)
                    {
                        if (EnableRotation == true)
                        {
                            ...
                            Vector3 intersectionScreenCenter =
                                GetIntersectionPoint(Cam.ScreenPointToRay(mouseRotationCenter));
                    ...
                    else if (isEditorInputTilt == true)
                    {
                        if (EnableTilt == true)
                        {
                            UpdateCameraTilt(Mathf.Sign(mouseScrollDelta) *
                                             Mathf.Min(25, Mathf.Abs(mouseScrollDelta * 100)));
                        }
                    }
                    else
                    {
            ...
            if (IsPinching == false && IsDragging == false)
            {
                ...
                {
                    float cameraSizeCorrection = Mathf.Lerp(0, camZoomDeltaToNormal,
                        zoomBackSpringFactor * Time.unscaledDeltaTime);
